...
def train(model, start, epochs, dl_train, dl_valid, log_step_freq, save_step_freq, logger, device, output_dir):
    ...
    train_steps = len(dl_train)
    val_steps = len(dl_valid)
    ...
    for epoch in range(start, epochs + 1):
        ...
        for step, batch in enumerate(dl_train, 1):
            ...
            if step % log_step_freq == 0:
                msg = "[Epoch {} | train {}/{}] loss: {:.3f} dice: {:.3f}".format(
                    epoch, step, train_steps, loss_sum / step, avd / step)
                logger.info(msg)
        ...
        for val_step, batch in enumerate(dl_valid, 1):
            ...
            if step % log_step_freq == 0:
                msg = "[Epoch {} | val {}/{}] loss: {:.3f} dice: {:.3f}".format(
                    epoch, val_step, val_steps, val_loss_sum / val_step, val_avd / val_step)
                logger.info(msg)
